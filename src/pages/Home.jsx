...
export default function Home() {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* === HERO BANNER 1 (FIXED PADDING & ALIGNMENT) === */}
      <div className="relative w-full h-[500px] md:h-[700px] lg:h-[800px] overflow-hidden mt-0 pt-0">
        ...
        {/* 🟢 FIXED: justify-end ensures text stays at the bottom */}
        <div className="absolute inset-0 flex flex-col items-start justify-end px-6 md:px-20 pb-10 md:pb-20">
          ...
          <Link 
            to="/men" 
            className="bg-white text-black font-bold py-3 px-8 md:py-4 md:px-10 rounded hover:bg-gray-100 hover:scale-105 transition-all duration-300"
          >
            SHOP NOW
          </Link>
        </div>
      ...
      <img 
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link to="/men" className="bg-black text-white font-semibold py-3 px-8 rounded hover:bg-gray-800 hover:scale-105 transition-all duration-300 border border-white/20">
              Shop Men
            </Link>
            <Link to="/women" className="bg-white text-black font-semibold py-3 px-8 rounded hover:bg-gray-100 hover:scale-105 transition-all duration-300">
              Shop Women
            </Link>
          </div>
        </div>
            ...
            {products.slice(0, 8).map((product) => (
              <Link to={`/product/${product._id}`} key={product._id} className="group cursor-pointer">
                <div className="relative overflow-hidden rounded-lg bg-gray-100 aspect-square border border-gray-100">
                  <img src={product.imageUrl || 'https://placehold.co/600x600/333/fff?text=Product+Image'} alt={product.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" onError={(e) => { e.target.src = 'https://placehold.co/600x600/333/fff?text=Image+Error'; }} />
