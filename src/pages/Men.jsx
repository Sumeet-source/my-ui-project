...
export default function Men() {
  ...
  const fetchMenProducts = async (page = 1, reset = false, filters = {}) => {
    ...
    } catch (error) { console.error('Error fetching men products:', error); if (reset) { setProducts([]); setFilteredProducts([]); setTotalCount(0); } } 
    finally { setLoading(false); setLoadingMore(false); }
  };

  useEffect(() => { fetchMenProducts(1, true); }, []);
  const handleLoadMore = () => { setLoadingMore(true); fetchMenProducts(currentPage + 1); };
  const applyFilters = (filters) => { setFilteredProducts([]); setHasMore(true); fetchMenProducts(1, true, filters); };
  const clearFilters = () => { setFilteredProducts([]); setHasMore(true); fetchMenProducts(1, true, {}); };

  return (
    // 🟢 Parent me px-0 hai, toh grid left/right se bilkul edge par rahega
    <div className="px-0 md:px-8 bg-white min-h-screen pb-10">
      
      {/* 🟢 FIXED STICKY BAR (Negative margin hata diya, ab right side gap zero) */}
      <div className="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-sm">
        {/* Inner centered container to keep text aligned with grid */}
        <div className="flex justify-between items-center py-3 px-4 md:px-10 max-w-[1280px] mx-auto">
          {/* Aapka pl-5 yahan bhi perfect chalega */}
      ...
      <FilterBottomSheet isOpen={isFilterOpen} onClose={() => setIsFilterOpen(false)} onApply={applyFilters} onClear={clearFilters} products={filteredProducts} defaultCategory="Men" totalCount={totalCount} />
    </div>
  );
}