...
export default function NewArrivals() {
  ...
  // Filtering logic for New Arrivals (Optionally filters by Men's or Women's)
  ...
  // Helper to set gender filter directly from the dropdown
  const setGenderFilter = (gender) => {
    setFilters((prev) => ({ ...prev, gender: gender ? [gender] : [] }));
  ...
  return (
    <div className="bg-white min-h-screen overflow-x-hidden relative">
      ...
      {/* --- MOBILE "NEW ARRIVALS" CATEGORY DROPDOWN (Image 1) --- */}
      {isCategoryOpen && (
        <div className="md:hidden bg-white border-b border-gray-200 px-6 py-4 flex flex-col gap-4 shadow-sm">
          <button onClick={() => setGenderFilter('men')} className="text-sm font-medium text-black text-left hover:text-blue-600">Men's</button>
          <button onClick={() => setGenderFilter('women')} className="text-sm font-medium text-black text-left hover:text-blue-600">Women's</button>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <h1 className="text-4xl font-bold text-gray-900 tracking-tight mb-8">FORGE New Arrivals</h1>
