...
export default function NewArrivalsFixed() {
  ...
  const fetchNewArrivals = async (page = 1, reset = false, filters = {}) => {
    ...
    try {
      ...
      // 🟢 If user selects a specific category via filters
  ...
  const applyFilters = (filters) => {
    setFilteredProducts([]);
    setHasMore(true);
    fetchNewArrivals(1, true, filters);
  };
  ...
  return (
    <div className="px-0 md:px-8 bg-white min-h-screen pb-10">
      ...
      {loading ? (
        <p className="text-center py-20 text-gray-500 text-sm mt-6">Loading new arrivals...</p>
      ) : (
        <div className="mt-6">
          {/* 👇 Nike-style perfect grid gap */}
