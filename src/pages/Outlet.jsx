...
export default function Outlet() {
  ...
  useEffect(() => { fetchOutletProducts(1, true); }, []);
  const handleLoadMore = () => { setLoadingMore(true); fetchOutletProducts(currentPage + 1); };
  const applyFilters = (filters) => { setFilteredProducts([]); setHasMore(true); fetchOutletProducts(1, true, filters); };
  const clearFilters = () => { setFilteredProducts([]); setHasMore(true); fetchOutletProducts(1, true, {}); };
  ...
  return (
    <div className="px-0 md:px-8 bg-white min-h-screen pb-10">
      ...
      <FilterBottomSheet isOpen={isFilterOpen} onClose={() => setIsFilterOpen(false)} onApply={applyFilters} onClear={clearFilters} products={filteredProducts} defaultCategory="Outlet" totalCount={totalCount} />
    </div>
  );
}