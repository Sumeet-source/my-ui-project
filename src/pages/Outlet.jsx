    ...
    <div className="bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <h1 className="text-3xl font-bold text-gray-900">Under Armour Outlet</h1>
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-12 gap-x-6">
            {sortedProducts.map((product) => (
              <ProductCard 
                key={product.id} 
                {...product} 
                originalPrice={product.originalPrice || product.price * 1.3} // Fake original price if not given
                discount={Math.round(((product.originalPrice || product.price * 1.3) - product.price) / (product.originalPrice || product.price * 1.3) * 100)}
                colors={product.colors || ['#000000', '#ffffff', '#cc0000']} // Fake colors if not given
              />
            ))}
          </div>
        </div>
