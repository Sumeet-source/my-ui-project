...
import { useWishlist } from '../context/WishlistContext.jsx'; // 🟢 Wishlist import
import { useToast } from '../context/ToastContext.jsx';
import { useAuth } from '../context/AuthContext.jsx'; // 🟢 Auth import for wishlist login check
...
export default function ProductDetails() {
  ...
  if (loading) {
    return <div className="text-center py-20 text-lg text-gray-500">Loading product...</div>;
  }
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      ...
      {/* --- Mobile Section (Myntra Style Layout) --- */}
        ...
        <div className="relative w-full bg-gray-50">
          ...
          <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5">
            {images.map((_, idx) => (
              <div 
                key={idx} 
                className={`w-1.5 h-1.5 rounded-full transition-colors ${mainImageIndex === idx ? 'bg-gray-900' : 'bg-gray-400'}`}
              />
            ))}
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              {['S', 'M', 'L', 'XL'].map((size) => (
                <button 
                  ...
                  className={`w-12 h-12 rounded-full border text-sm font-medium transition ${selectedSize === size ? 'border-black bg-black text-white' : 'border-gray-300 text-gray-700 hover:border-black'}`}
                >
                  {size}
                </button>
              ))}
            ...
            {!product.inStock ? (
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  className="flex-1 bg-black text-white py-4 rounded-md font-bold text-sm tracking-wide hover:bg-gray-800 transition"
                >
                  ADD TO BAG
                </button>
                <button 
                  onClick={handleWishlistToggle}
                  className="w-14 h-14 border border-gray-200 rounded-md flex items-center justify-center text-gray-700 hover:bg-gray-50 transition"
                >
        ...
        <div className="px-4 py-4 border-t border-gray-100 bg-gray-50">
          ...
          <div className="bg-white border border-gray-200 rounded p-4">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-3">
              ...
              <select value={newReview.rating} onChange={(e) => setNewReview({...newReview, rating: Number(e.target.value)})} className="w-full border border-gray-300 rounded p-2 text-sm focus:outline-none focus:border-black">
