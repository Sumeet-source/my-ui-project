...
export default function Shoes() {
  ...
  const handleGenderChange = (gender) => {
    ...
    if (gender === 'All') {
      // Remove gender from URL when 'All' is clicked
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-sm">
        <div className="py-3 px-4 md:px-10 max-w-[1280px] mx-auto">
          ...
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setIsFilterOpen(true)} className="flex items-center gap-2 border border-gray-300 bg-white px-4 py-1.5 rounded-full text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
              <Filter className="w-4 h-4" /> Filter
            </button>
          </div>
          
          {/* 🟢 Gender Tabs - All always highlighted by default */}
          <div className="flex gap-2 px-4 md:px-0 mt-2 overflow-x-auto">
            {['All', 'Men', 'Women'].map((gender) => {
              // Active condition:
              // - 'All' is active when no gender in URL OR gender === 'All'
              // - 'Men'/'Women' active only when URL matches
              const isActive = 
                (gender === 'All' && !genderFromUrl) || 
                (gender !== 'All' && genderFromUrl === gender);
              
              return (
                <button
                  key={gender}
                  onClick={() => handleGenderChange(gender)}
                  className={`px-3 py-1 text-sm font-medium transition-all duration-200 whitespace-nowrap border rounded-full ${
                    isActive
        ...
        <div className="mt-0 md:mt-2">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-8">
            {filteredProducts.length === 0 ? (
              <p className="col-span-full text-center py-20 text-gray-500 text-sm">No shoes match your filters.</p>
            ) : (
          ...
          {hasMore && !loadingMore && (
            <div className="flex justify-center mt-10">
              <button onClick={handleLoadMore} className="px-8 py-2.5 border border-black text-black text-sm font-medium rounded-full hover:bg-black hover:text-white transition-colors duration-300">
                Load More
              </button>
            </div>
          )}
          {loadingMore && (
            <p className="text-center mt-10 text-sm text-gray-500">Loading more products...</p>
          )}
        </div>
      )}
      <FilterBottomSheet
