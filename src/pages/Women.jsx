    ...
    <div className="bg-white min-h-screen overflow-x-hidden relative">
      <div className="md:hidden flex justify-between items-center border-b border-gray-200 py-3 px-4 bg-white shrink-0">
        <button onClick={() => setIsCategoryOpen(!isCategoryOpen)} className="flex-1 flex items-center justify-center gap-1 text-sm font-medium text-black border-r border-gray-200">
          Women {isCategoryOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
      ...
      {isCategoryOpen && (
        <div className="md:hidden bg-white border-b border-gray-200 px-6 py-4 flex flex-col gap-4 shadow-sm">
          <Link to="/women" onClick={() => setIsCategoryOpen(false)} className="text-sm font-medium text-black hover:text-blue-600">Featured</Link>
          <Link to="/women" onClick={() => setIsCategoryOpen(false)} className="text-sm font-medium text-black hover:text-blue-600">Shop by Category</Link>
          <Link to="/women" onClick={() => setIsCategoryOpen(false)} className="text-sm font-medium text-black hover:text-blue-600">Shop by Sport</Link>
          <Link to="/women" onClick={() => setIsCategoryOpen(false)} className="text-sm font-medium text-black hover:text-blue-600">Shop by Collection</Link>
          <Link to="/women" onClick={() => setIsCategoryOpen(false)} className="text-sm font-medium text-black hover:text-blue-600">Accessories</Link>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <h1 className="text-4xl font-bold text-gray-900 tracking-tight mb-8">FORGE Women's</h1>
